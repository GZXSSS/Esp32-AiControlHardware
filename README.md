...
**Esp32-AiControlHardware** 是一个基于 ESP32 的智能硬件控制平台，通过集成大语言模型 API，实现自然语言驱动的设备控制。用户可以通过语音、文本或串口指令操作硬件，系统自动解析并执行对应的 GPIO 操作，让智能家居控制更加自然、便捷。
...
4. 重启后进入 AP 模式，热点名称为 `ESP32_Config`（或类似预设 SSID）。
...
- 发送 `TOGGLE` → 翻转 D2 引脚当前状态
- 发送 `STATUS` → 查询当前 GPIO 状态（返回引脚电平信息）
- 发送 `RESET` → 软件复位 ESP32

（*提示：你也可以在 `main` 目录的串口处理逻辑中自由增删自定义指令。*）
